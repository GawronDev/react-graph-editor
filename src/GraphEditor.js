...
export default function GraphEditor(props) {
    ...
    */
    ...
    document.addEventListener("keydown", (event) => {
        ...
        if(event.key === "n" && event.ctrlKey && !key_pressed){
            key_pressed = true;
            addNode();
        }
        if(event.key === "t" && event.ctrlKey && !key_pressed){
            key_pressed = true;
            topsort();
        }
        if(event.key === "c" && event.ctrlKey && !key_pressed){
            key_pressed = true;
            setOpenDialog(true);
        }
        if(event.key === "r" && event.ctrlKey && !key_pressed){
            key_pressed = true;
            clearNodes();
        }

        if(openDialog && event.key === "Escape" && !key_pressed){
            key_pressed = true;
            setOpenDialog(false);
        } 
      });
    ...
    return (
        <div className="general-wrapper">
            ...
            <Sidebar clear_nodes={clearNodes} new_node={addNode} openConnectDialog={openConnectDialog} topsort={topsort} name={props.name} email={props.email} profile_picture={props.profile_picture} logout={props.logout}/>
